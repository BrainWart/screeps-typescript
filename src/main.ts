...
export const loop = ErrorMapper.wrapLoop(() => Timer.log(logger, () => {
  ...
  for (const roomName in Game.rooms) {
    ...
    if (room.controller && room.controller.my) {
      ...
      for (const tower of room.find<StructureTower>(FIND_MY_STRUCTURES, { filter: (s) => s instanceof StructureTower })) {
            ...
            if (workerLimits[job as Tasks] > 0) {
              ...
              spawn.spawnCreep([WORK, MOVE, MOVE, CARRY, CARRY], potentialCreepName, { memory: { task: {task: "upgrade", room: roomName, working: false} }});
                ...
                case "build":
                  ...
                  break;
                case "spawn":
                  spawn.spawnCreep([MOVE, MOVE, CARRY, CARRY], potentialCreepName, { memory: { task: {task: "spawn", room: roomName, working: false} }});
                  break;
              }
            }
          }
  ...
  for (const creepName in Memory.creeps) {
    ...
    switch (creep.memory.task.task) {
      case "upgrade":
        new UpgradeTask(creep, creep.memory.task, logger).act();
        break;
      case "harvest":
        new HarvestTask(creep, creep.memory.task, logger).act();
        break;
      case "build":
        new BuildTask(creep, creep.memory.task, logger).act();
        break
      case "spawn":
        new SpawnTask(creep, creep.memory.task, logger).act();
        break
      case "attack":
        new AttackTask(creep, creep.memory.task, logger).act();
        break
      default:
