...
function getEnergySource(room: Room): Resource<ResourceConstant> | AnyStoreStructure | Tombstone | undefined {
  ...
  return sources.length > 0 ? _.max(sources, (r) => ("amount" in r && r.amount) || ("store" in r && r.store.getUsedCapacity("energy"))) : undefined;
}

function getSpawnRelated(room: Room): StructureSpawn | StructureExtension | StructureTower | undefined {
  return _.first([
    ...room.find<StructureTower>(FIND_MY_STRUCTURES, { filter: (s) => s instanceof StructureTower }),
    ...room.find(FIND_MY_SPAWNS),
    ...room.find<StructureExtension>(FIND_MY_STRUCTURES, { filter: (s) => s instanceof StructureExtension }),
  ]);
}

export class SpawnTask extends Task<SpawnMemory> {
  constructor(creep: Creep, memory: SpawnMemory, logger: Logger) { super(creep, memory, logger); }
    ...
    if (this.memory.working) {
      ...
      if (toFill) {
        if (this.creep.pos.isNearTo(toFill)) {
            this.creep.transfer(toFill, RESOURCE_ENERGY, Math.min(this.creep.store.getUsedCapacity(RESOURCE_ENERGY), toFill.store.getFreeCapacity(RESOURCE_ENERGY)));
